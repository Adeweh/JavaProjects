...
public class KeyGenerator {
    private static final char[] newArray = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".toCharArray();
    private static final List<Integer> remainderList = new ArrayList<>();

    public static String convertKey(int key) {
        if (key < 62) return String.valueOf(newArray[key]);
        ...
        for (Integer remainder : remainderList) {
            convertedKey.append(newArray[remainder]);
        }
        remainderList.clear();
    ...
    public static int getKey(String convertedKey) {
        ...
        for (int i = 0; i < convertedKey.length(); i++) {
            for (int j = 0; j < 62; j++) {
                if (convertedKey.charAt(i) == newArray[j]) {
